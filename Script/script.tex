...
\title{lumpR: An R package facilitating landscape discretisation for hillslope-based hydrological models}
...
\runningtitle{lumpR: R package for landscape discretisation}
...
Therefore, compromises between the two extremes, sometimes also referred to as \emph{semi-distributed} approaches, are often the preferred choice in practical applications \citep[e.g.,][]{Kumar2010, Euser2015}.
...
Thus, the properties of both distributed and lumped modelling can be found.
...
In the last decades, a large number of discretisation schemes for semi-distributed models has been developed along with an even larger number of software solutions.
...
Therefore, the first objective of this paper is to provide an overview of existing landscape discretisation algorithms and software implementations for semi-distributed hydrological model application.

As will be further discussed in Sect. \ref{sec:review}, among the semi-distributed approaches, hillslope-based modelling is an efficient option for representing heterogeneous runoff generation processes while accounting for phenomena of hydrological connectivity.
However, so far only few computer programmes exist that aid in the pre-processing of hillslope-based models.
Furthermore, these are often model-specific, have a limited applicability, and are not freely available or can only be used along with commercial software.
Therefore, the second objective is to present a new software package for the pre-processing of hillslope-based multi-scale hydrological models addressing these limitations.
...
Thus, the third and last objective of this paper is to present an example case study in a semi-arid catchment using the WASA-SED model, thereby conducting a sensitivity analysis of crucial discretisation parameters (Sects. \ref{sec:case_study}).

Eventually, the findings of this study are discussed and conclusions are inferred (Sect. \ref{sec:discussion} and \ref{sec:conclusions}, respectively).
