...
%DIF ADD script_rev.tex   Mon Apr 10 09:49:39 2017
%% ---------------------------------
...
%DIF -------
 %DIF > 
%DIF < % tabularx: vertical alignment (p - top, m - center, b - bottom) in multi-column tabularx cells
 %DIF > 
%DIF < \def\tabularxcolumn#1{p{#1}} 
...
Thus, the properties of both distributed and lumped modelling can be found\DIFaddbegin \DIFadd{, often extended by integrating subgrid parameterisation schemes}\DIFaddend .
...
\DIFaddbegin \DIFadd{As a basis for watershed delineation and further landscape discretisation, a number of pre-processing steps have to be performed.
...
%DIF > \begin{tabularx}{\textwidth}{L{.2}L{.16}L{.22}L{.2}L{.22}}
...
\DIFaddFL{flow-interval scheme }& \DIFaddFL{Hillslope }& \DIFaddFL{-- }& \DIFaddFL{-- }& \DIFaddFL{\mbox{%DIFAUXCMD
...
With the latter we mean conceptions exceeding the common }\emph{\DIFadd{watershed > subbasin > element}} \DIFadd{scheme.
...
\citet{Yang2002} }%DIFAUXCMD
introduced the }\emph{\DIFadd{flow-interval hillslope scheme}} \DIFadd{where a catchment is subdivided into a number of connected flow intervals which are defined by the width (i.e., the number of streams) and the geomorphological area (i.e., the drainage area) as functions of distance from the watershed outlet.
...
\DIFaddbegin \DIFadd{On the other hand, especially when iterating through individual hillslopes of a watershed, applicability is clearly limited to smaller scales due to the large computational demand.
}\DIFaddend 
...
\DIFdelbegin \DIFdel{Their software is }\DIFdelend \DIFaddbegin \DIFadd{Contrary to other hillslope-based algorithms, due to the discretisation at multiple scales, it is applicable over large areas with relatively little effort.
Even though LUMP was directed to the pre-processing of the WASA-SED model \mbox{%DIFAUXCMD
...
\DIFaddbegin \DIFadd{Figure \ref{fig:lumpr_output} gives an illustrative example for the outcomes of the following steps (i) to (iv).
}\DIFaddend \emph{(i) calc\_subbas()} sub-divides the hydrological basin into subbasins using a given grid-based DEM \DIFdelbegin \DIFdel{.
...
\DIFdelbegin \DIFdel{Figure \ref{fig:lumpr_output} gives an illustrative example for the outcome after applying steps (i) to (iv).
}\DIFdelend 
...
In addition, }\DIFaddend subbasin and LU specific parameters \DIFdelbegin \DIFdel{thereby employing }\DIFdelend \DIFaddbegin \DIFadd{such as representative channel geometry and routing parameters, groundwater, and landscape coefficients, are approximated.
...
\DIFdelbeginFL %DIFDELCMD < \begin{tabularx}{\columnwidth}{L{.25}L{.45}C{.3}}
%DIFDELCMD < %%%
...
\DIFdelbeginFL %DIFDELCMD < \begin{tabularx}{\textwidth}{L{.1}L{.25}L{.5}L{.15}}
%DIFDELCMD < %%%
...
Future work\DIFaddbegin \DIFadd{, on the one hand, }\DIFaddend will focus on comparing uncertainties arising from \DIFdelbegin \DIFdel{discretization }\DIFdelend \DIFaddbegin \DIFadd{discretisation }\DIFaddend to other sources of uncertainties.
\DIFaddbegin \DIFadd{On the other hand, in order to obtain more general conclusions, the presented sensitivity analysis of landscape discretisation parameters needs to be extended to other catchments within different environmental and hydro-meteorological conditions as well as other hillslope-based models.
}\DIFaddend Technical extensions will include the integration of further models, improvement of time consumption and memory handling for application in large areas > 100,000 \unit{km^2}, consideration of artificial discontinuities and mechanisms for large flat areas, refinement of certain parameter estimation approaches, and testing the package for other hydro-meteorological and environmental conditions.
...
\DIFdelbegin \DIFdel{M. Sc. student }\DIFdelend \DIFaddbegin \DIFadd{Student }\DIFaddend Lisa Bergh\"auser from the University of Potsdam is acknowledged for testing lumpR \DIFdelbegin \DIFdel{within her Thesis }\DIFdelend and giving valuable feedback and bug reports.
