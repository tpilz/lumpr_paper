...
Even though LUMP was directed to the pre-processing of the WASA-SED model \citep{Guentner2004b}, it is stand-alone and the output can as well be used for other hillslope-based models.
...
In addition, subbasin and LU specific parameters such as representative channel geometry and routing parameters, groundwater, and landscape coefficients, are approximated.
...
Instead of the simulated river discharge (i.e., the model output used to calculate the streamflow indices, see Sect.\ \ref{sec:scalar_targetvar}), Fig.\ \ref{fig:resvol} provides an overview over the simulated reservoir storages in comparison to observations for the Bengu\^e reservoir at the catchment outlet.
...
Student Lisa Bergh\"auser from the University of Potsdam is acknowledged for testing lumpR and giving valuable feedback and bug reports.
