...
\citet{Yang2002} introduced the \emph{flow-interval hillslope scheme} where a catchment is subdivided into a number of connected flow intervals which are defined by the width (i.e., the number of streams) and the geomorphological area (i.e., the flow area) as functions of distance from the watershed outlet.
...
On the other hand, when iterating through individual hillslopes of a watershed, applicability is clearly limited to small scales due to the large computational demand.
...
Contrary to other hillslope-based algorithms, due to the discretisation at multiple scales, it is applicable over large scales with relatively little effort.
